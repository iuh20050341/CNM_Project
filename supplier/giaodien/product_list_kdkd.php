...
<body>
    ...
    if (isset($_SESSION['ten_dangnhap']) && !empty($_SESSION['ten_dangnhap']) && $_SESSION['user_id']) {
        ...
        if (isset($_POST['search'])) {
            $sql = "SELECT * FROM `sanpham` WHERE id_nhaban = $user_id AND trangthai = 5";
        ...
        <div class="main-content" style="color: green">
            <h1>Danh sách sản phẩm không đạt chuẩn</h1>
            <form method="POST" action="./supplier.php?tmuc=SP%20chưa%20đạt">
                <div class="form-row">
                                ...
                                <tr>
                                    ...
                                    <td style="text-align:center; padding-top: 50px">
                                        <form method="POST" action="xulythem.php">
                                            ...
                                            <button type="submit" name="btngui" onclick="return confirm('Bạn có muốn gửi yêu cầu kiểm định?')">Gửi lại yêu cầu kiểm định</button>
                                        </form>
