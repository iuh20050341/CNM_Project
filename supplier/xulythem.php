    ...
    if (isset($_POST['saveAdd'])) {
        if (isset($_POST['diachivuon']) && isset($_POST['id'])) {
            if ($_POST['diachivuon'] != '') {
                ...
                $result = mysqli_query($conn, "UPDATE `khachhang` SET `diachivuon` = '" . $_POST['diachivuon'] . "'WHERE `id` = " . $_POST['id'] . " ");
                if ($result) {
                    echo "<script>alert('Cập nhật địa chỉ thành công!');
                ...
                echo "<script>alert('Vui lòng nhập địa chỉ!');
                    window.location.href = 'supplier.php';</script>";
            }
        }
    }
